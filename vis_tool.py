...
def vis_train(img, bbox, label, scale, hbb=True):
    ...
    for i in range(len(bbox)):
        ...
        height = y2 - y1
        width = x2 - x1
        ax.add_patch(plot.Rectangle((x1,y1), width, height, fill=False, edgecolor='red', linewidth=2))
        ax.text(x1,y1, DOTAv10_BBOX_LABEL_NAMES[label[i]],style='italic',bbox={'facecolor': 'white', 'alpha': 0.5, 'pad': 0})
    
    ax.text(10,10,'Scale='+str(scale.numpy()),style='italic',bbox={'facecolor': 'white', 'alpha': 0.5, 'pad': 0})
    plot.show()
    return ax
...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    label_names = list(DOTAv10_BBOX_LABEL_NAMES) + ['bg']
    # add for index `-1`
